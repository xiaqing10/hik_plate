...
void CALLBACK MSesGCallback(LONG lCommand, NET_DVR_ALARMER *pAlarmer, char *pAlarmInfo, DWORD dwBufLen, void* pUser)
{
	...
	case COMM_ITS_PLATE_RESULT: {
		...
		{
            auto sLicense = GBKToUTF8(struITSPlateResult.struPlateInfo.sLicense).c_str();
			...
			// {
			// 	sprintf(filename, "./pic/%s_%d.jpg", sLicense.c_str(), i);
			...
			// {
			// 	sprintf(filename, "./pic/snap/%s_%d.jpg", sLicense.c_str(), i);
            ...
            // std::cout << sLicense << " 车牌置信度: " << byEntireBelieve << " 车辆类型: "<< byVehicleType << " 车身颜色深浅: " <<byColorDepth \
            //  << " 车身颜色: " << byColor << " 车辆品牌: " << byIllegalType <<  " 车辆子品牌:  " << byVehicleLogoRecog << std::endl;

			// 被触发的出道号
			// std::cout << "byDriveChan  No: " << struITSPlateResult.byDriveChan  << std::endl;

            plateInfo plate;
            // if (byEntireBelieve == 0)  break;

            plate.sLicense = sLicense;
			...
			plate.byDriveChan = struITSPlateResult.byDriveChan;
...
int main(){
    ...
    if (PRODUCER_INIT_SUCCESS == producer->init_kafka(0, (char*)"10.10.31.97:9092", (char*)"plate-topic"))  
    {  
    ...
    while(1){
        ...
        {	 
			...
			plateObj["xmin"] = info.xmin;
			plateObj["xmax"] = info.xmax;
			plateObj["ymin"] = info.ymin;
			plateObj["ymax"] = info.ymax;
			plateObj["sLicense"] = info.sLicense;	
			plateObj["byEntireBelieve"] = info.byEntireBelieve;
			plateObj["byVehicleType"] = info.byVehicleType;
			plateObj["byColorDepth"] = info.byColorDepth;
			plateObj["byColor"] = info.byColor;
			plateObj["byIllegalType"] = info.byIllegalType;
			plateObj["byVehicleLogoRecog"] = info.byVehicleLogoRecog;
			plateObj["byVehicleSubLogoRecog"] = info.byVehicleSubLogoRecog;
			plateObj["byDriveChan"] = info.byDriveChan;

			root["plateResult"] = plateObj;
